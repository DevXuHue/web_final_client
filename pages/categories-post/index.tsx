...
import { CLEAR_GET_ALL_CATEGORY_POST_ERROR } from "@/constants";
import { useAppDispatch, useAppSelector } from "@/hooks";
import LayoutAdmin from "@/layouts";
import { parseTime } from "@/libs/helper";
import { fetchAllCategoiesPost } from "@/redux/actions";
...
const categoriesPost = () => {
  ...
  const { loading, categoryPosts, error } = useAppSelector(
    (state) => state.getAllCategoryPostReducer
  );

  useEffect(() => {
    if (error) {
      toast.error("Có lỗi đã xảy ra!!");
      dispatch({ type: CLEAR_GET_ALL_CATEGORY_POST_ERROR });
      router.push("/");
    }
  }, [error]);
  ...
  useEffect(() => {
    // @ts-ignore
    dispatch(fetchAllCategoiesPost());
  }, []);
    ...
    <>
      {loading ? (
        ...
        <LayoutAdmin>
          ...
          <button
            onClick={() => router.push("/categories-post/new")}
