...
import { CLEAR_GET_ALL_REPORT_TYPE } from "@/constants";
import { useAppDispatch, useAppSelector } from "@/hooks";
import LayoutAdmin from "@/layouts";
import { parseTime } from "@/libs/helper";
import { getReportTypes } from "@/redux/actions";
import { useRouter } from "next/router";
import { useEffect } from "react";
import toast from "react-hot-toast";

const TypesReportPage = () => {
  const router = useRouter();
  const dispatch = useAppDispatch();

  const { loading, report_types, error } = useAppSelector(
    (state) => state.getReportTypesReducer
  );
                    ...
                    <table className="min-w-full ">
                      ...
                      <tbody>
                        {report_types?.length > 0 ? (
                          report_types.map((item: any, index: number) => (
...
export default TypesReportPage;
